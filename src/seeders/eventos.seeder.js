...
const insertarEventos = async () => {
    const query = 'INSERT INTO eventos (id_bovino, titulo, asunto, descripcion, fecha_Reinsidio, created_by, fecha_Reporte) VALUES (?, ?, ?, ?, ?, ?, ?)';

    for (const evento of eventos) {
        const [created_byResult] = await db.execute('SELECT id_administrador FROM administradores WHERE correo = ? limit 1', [evento.created_bySub]);
